...
export function registerWasteRoutes(app:FastifyInstance){
 ...
 // Palvelimen oma viesti menee sellaisenaan selaimeen. Se on ainoa kerros joka
 // tietää MIKÄ meni pieleen (huoltokatko, verkko, hylätyt tunnukset), ja yleisteksti
 // sen tilalla sanoi aiemmin "tarkista tunnukset" myös silloin kun kyse oli yhtiön
 // huoltokatkosta — juuri se kehotus ajaa salasanojen kokeiluun ja lukitsee tilin.
 app.post('/api/waste/properties',async(req,reply)=>{if(!allowed(req,reply))return reply;try{return {properties:await wasteProperties()};}catch(error){const wait=(error as {retryAfterSeconds?:number}).retryAfterSeconds;if(wait)reply.header('retry-after',String(wait));return reply.code((error as {statusCode?:number}).statusCode===429?429:400).send({error:error instanceof Error?error.message:'Yhdistäminen epäonnistui.',...(wait?{retryAfterSeconds:wait}:{})});}});
 ...
 if(result.outcome==='failed')return reply.code(502).send({error:result.error.message});
